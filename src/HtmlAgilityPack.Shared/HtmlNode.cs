...
namespace HtmlAgilityPack
{
	...
	public partial class HtmlNode
	{
		...
		/// 获取为特定元素节点定义特定行为的标志集合。
		/// 该表包含一个DictionaryEntry列表，其键为小写标记名，值为HtmlElementFlags的组合。
		/// </summary>
		...
		/// 获取或设置“id”HTML属性的值。文档必须使用设置为true的OptionUseIdAttribute进行解析。
		/// </summary>
		public string Id
		{
			get
        ...
        internal virtual void InternalInnerText(StringBuilder sb, bool isDisplayScriptingText, int depthLevel)
        {
			...
			//注意:现在，这个方法是*slow*的，因为我们需要重新计算所有内容。
			//它可以像innerhtml一样优化
		...
		/// 获取 <see cref="HtmlDocument"/>节点所属的节点。
		/// </summary>
		...
		/// 节点相对于开始的根html元素的深度。此值用于确定文档是否有许多可能导致堆栈溢出的嵌套html节点
		/// </summary>
		...
		/// </summary>
		/// <param name="html">The HTML text.</param>
		/// <param name="htmlDocumentBuilder">The HTML Document builder.HTML文档构建器</param>
		/// <returns>The newly created node instance.</returns>
		...
		/// 如果名称是CDATA元素节点的名称，则为true，否则为false。
		/// </returns>
		...
		/// True递归地将子树克隆到指定节点下;仅克隆节点本身为False。
		/// </param>
		/// <returns>The cloned node.</returns>
		...
		/// 方法来获取此节点的属性的值。如果没有找到该属性，将返回默认值。
		/// </summary>
		/// <param name="name">
		/// The name of the attribute to get. May not be <c>null</c>.
		/// 要获取的属性的名称,不能是<c>null</c>
		/// </param>
		/// <param name="def">
		...
		/// 方法来设置此节点的属性的值。如果没有找到该属性，将自动创建它。
		/// </summary>
		...
		/// 设置父Html节点并使用父节点的深度正确地确定当前节点的深度。
		/// </summary>
